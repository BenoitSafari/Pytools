...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "-o", "--output", type=Path, help="Output directory (default: ./<basename>/)"
    )
    ...
    parser.add_argument(
        "--partition", default="secure", help="Partition to extract (default: secure)"
    )
    ...
    with open(args.input, "rb") as fh:
        ...
        for name, part in xci.partitions.items():
            ...
            print(f"  Partition '{name}': {file_count} files, {format_size(total_size)}")
        ...
        if partition_name not in xci.partitions:
            ...
            print(
                f"Error: partition '{partition_name}' not found. Available: {available}",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if args.nsp:
            ...
            print(f"NSP written: {nsp_path} ({format_size(nsp_path.stat().st_size)})")
